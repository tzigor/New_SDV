...
uses
  ...
  Controls, Dialogs, DateUtils, GraphUtil;
...
procedure SerieShiftTime(Serie : TLineSeries; Hrs, Mins, Secs: Integer);
...
var n    : LongWord;
    XVal : Double;
begin
  if (Hrs <> 0) Or (Mins <> 0) Or (Secs <> 0) then begin
    for n:=0 to Serie.Count - 1 do begin
       XVal:= Serie.GetXValue(n);
       XVal:= IncHour(XVal, Hrs);
       XVal:= IncMinute(XVal, Mins);
       XVal:= IncSecond(XVal, Secs);
