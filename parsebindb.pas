...
begin
   ...
   if isEcoscope then IncDataOffset(1);
end;
...
begin
  AssignFile(FS, 'SDVDebugFile.txt');
  Rewrite(FS);
  isEcoscope:= False;
  ErrorCode:= NO_ERROR;
  isEnergiya:= False;
     ...
     if Not EndOfFile then begin
         case RecordType of
            'P': begin
                    ...
                    if NPos('DVD6', NewParameter, 1) > 0 then
                      isEcoscope:= True;
            ...
            'D': begin
                    ...
                    Inc(ChannelCount);
                    Writeln(FS, AddCharR(' ', IntToStr(ChannelCount), 4) + ' - ' + AddCharR(' ', DataChannel.DLIS, 10) + ': ' + AddCharR(' ', DataChannel.Units, 4) + ': ' + AddCharR(' ', DataChannel.RepCode, 3) + ': ' + AddCharR(' ', DataChannel.Samples, 4) + ': '  + DataChannel.AbsentValue);
                    FrameLength += GetTypeLength(DataChannel.RepCode);
                 end;
            'F': begin
  ...
  TffFrames.Done;
  ProgressDone();
  Writeln(FS, 'Frame length = ' + IntToStr(FrameLength));
  CloseFile(FS);
end;
