...
begin
  ...
  if Channel = 0 then Value:= Frame.DateTime
    ...
    'F4': begin
            ...
            if IsNan(F4) then Result:= False
            else if (F4 = -999.25) then Result:= False
                 else Value:= F4;
          end;
    'F8': begin
            Move(Bytes[DataOffset], F8, 8);
            if IsNan(F8) then Result:= False
            else if (F8 = -999.25) Or (F4 = Nan) or (F4 = +infinity) or (F4 = -infinity) then Result:= False
                 else Value:= F8;
          end;
    'I1': begin
...
var i: Integer;
begin
  for i:=1 to MAX_CHART_NUMBER do GetChart(i).DisableRedrawing;
end;
